// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"Konnect/internal/sdk/pkg/models/shared"
	"net/http"
)

type PatchSystemAccountsIDAccessTokensIDRequestBody struct {
	// Name of the system account access token.
	Name *string `json:"name,omitempty"`
}

type PatchSystemAccountsIDAccessTokensIDRequest struct {
	RequestBody *PatchSystemAccountsIDAccessTokensIDRequestBody `request:"mediaType=application/json"`
	// ID of the system account.
	AccountID string `pathParam:"style=simple,explode=false,name=accountId"`
	// ID of the system account access token.
	TokenID string `pathParam:"style=simple,explode=false,name=tokenId"`
}

type PatchSystemAccountsIDAccessTokensIDResponse struct {
	// Conflict
	ConflictError *shared.ConflictError
	ContentType   string
	// Not Found
	NotFoundError *shared.NotFoundError
	StatusCode    int
	RawResponse   *http.Response
	// A response including a single system account access token.
	SystemAccountAccessToken *shared.SystemAccountAccessToken
	// Unauthenticated
	UnauthorizedError *shared.UnauthorizedError
}
