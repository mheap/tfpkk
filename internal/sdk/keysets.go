...
func (s *KeySets) CreateKeySet(ctx context.Context, request operations.CreateKeySetRequest) (*operations.CreateKeySetResponse, error) {
	...
	url, err := utils.GenerateURL(ctx, baseURL, "/control-planes/{controlPlaneId}/core-entities/key-sets", request, nil)
	if err != nil {
	...
	case httpRes.StatusCode == 400:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out operations.CreateKeySetResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
...
func (s *KeySets) DeleteKeySet(ctx context.Context, request operations.DeleteKeySetRequest) (*operations.DeleteKeySetResponse, error) {
	baseURL := utils.ReplaceParameters(s.sdkConfiguration.GetServerDetails())
	url, err := utils.GenerateURL(ctx, baseURL, "/control-planes/{controlPlaneId}/core-entities/key-sets/{KeySetId}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
